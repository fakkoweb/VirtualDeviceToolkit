...
IProbe<raw_elem_type, refined_elem_type>::IProbe
	(
	...
	) : dynamic_sample_rate(false)
{
	...
	IProbe(selected_polling, selected_processing, mean_interval, enable_mean_offset);
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::reset()
{
	...
	statistic.expected_samples = statistic_delay.count() / sample_delay.count(); 	//An ESTIMATION of number of expected samples to be picked from IInputDevice;
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::closeSamplingThread()
{
	std::unique_lock<std::mutex> access(rw, std::defer_lock);
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::refresh()		//This function is called manually or automatically, in which case all sampling operation must be ATOMICAL 
{
	std::unique_lock<std::mutex> access(rw, std::defer_lock);
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::publish_measure()
{
	...
	if (raw_measure != VDT_VALUE_INVALID)	//ONLY IF MEASURE IS VALID..
	{
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::publish_statistic()
{
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::update_statistic()
{
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::wait_new_sample()
{
	if (polling_mode == automatic) measureProvisioning.waitForNextUpdate();
}

template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::wait_new_statistic()
{
	statisticProvisioning.waitForNextUpdate();
}

template <class raw_elem_type, class refined_elem_type>
bool IProbe<raw_elem_type, refined_elem_type>::new_sample_is_ready()
{
	return measureProvisioning.checkForUpdates();
}

template <class raw_elem_type, class refined_elem_type>
bool IProbe<raw_elem_type, refined_elem_type>::new_statistic_is_ready()
{
	return statisticProvisioning.checkForUpdates();
}

template <class raw_elem_type, class refined_elem_type>
raw_elem_type IProbe<raw_elem_type, refined_elem_type>::get_raw(sync_policy_t sync_mode)
{
	raw_elem_type measure = VDT_VALUE_INVALID;
...
template <class raw_elem_type, class refined_elem_type>
void IProbe<raw_elem_type, refined_elem_type>::plug_to(const IInputDevice<void, raw_elem_type>& new_board, const std::chrono::system_clock::time_point& start_time)
{
	std::lock_guard<std::mutex> access(rw);
	...
	//Set new board
	board = const_cast< IInputDevice<measure_struct, raw_elem_type>* > (&new_board);
